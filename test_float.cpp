...
void test_cond(cond_type type) {
...
#endif
	{
		...
		{
		case cond_type::less:
			coat::if_then(fn, data < 10.0f, [&] {
				sum = 1.0f;
			});
			expected = 1.0f;
			coat::if_then_else(fn, data < sum + sum,
			[&] {
		...
		case cond_type::great:
			coat::if_then(fn, data > 4.0f, [&]{
				sum = 1.0f;
			});
			expected = 1.0f;
			coat::if_then_else(fn, sum > data,
			[&] {
		...
		case cond_type::equal:
			...
			expected = 5.0f;
			coat::if_then_else(fn, data != 5.0f,
			[&]{
			},
			[&]{
				sum += 5.0f;
